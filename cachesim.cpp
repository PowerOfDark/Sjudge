...
void cachesim::init()
{
    ...
    {
        fwrite("PIN_DefineTraceBuffer failed\n", 1u, 0x1Du, stderr);
...
VOID* cachesim::trace_buffer_full(BUFFER_ID id, THREADID tid, const CONTEXT* ctxt, VOID* buf, UINT64 numElements, VOID* v)
{
    ...
    while(left > 0)
    {
        ...
        if(valhigh > 0x40)
        {
            fprintf(stderr, "what in the hell %u\n", valhigh);
...
VOID cachesim::thread_start(THREADID threadIndex, CONTEXT *ctxt, int flags, VOID *v)
{
    ...
    {
        fwrite("fatal: maximum number of threads exceeded", 1u, 0x29u, stderr);
        PIN_ExitProcess(0xFA);
    }
}
